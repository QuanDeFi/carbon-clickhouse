...
| **Rust toolchain (1.88.0+)** | Installs `rustc` and `cargo`, which build and run the example binary. |
...
* `DATASOURCE` toggles which Carbon datasource powers the pipeline. Leave it at `rpc_transaction_crawler` for the historical/default behavior (signature polling), or switch to `rpc_block_crawler` to stream blocks as they are produced. The block crawler starts near the head (`latest_slot - 1`) and moves forward.
* `RATE_LIMIT` behavior depends on datasource:
  - `rpc_transaction_crawler`: defaults to 10 requests/s if `RATE_LIMIT` is not set.
  - `rpc_block_crawler`: request throttling is only applied when `RATE_LIMIT` is set.
...
1. Logging is set up (stdout + timestamped `/logs/run-*.log` files).
...
3. The datasource selected via `DATASOURCE` is initialized:
   ...
   - `RpcBlockCrawler` fetches the most recent slot via `getSlot`, starts from `latest_slot - 1`, and then streams blocks forward.
...
| `0 processed` forever | RPC endpoint not returning Jupiter transactions or free-tier restrictions on required methods | Verify RPC connection, run `curl getSignaturesForAddress JUP6...`, upgrade RPC plan or switch providers. |
