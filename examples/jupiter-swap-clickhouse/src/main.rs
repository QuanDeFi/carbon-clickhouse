...
use {
    ...
    carbon_jupiter_swap_decoder::{
        instructions::clickhouse::{
            bootstrap_clickhouse_from_database_url, default_clickhouse_processor,
        },
        JupiterSwapDecoder,
    },
...
pub async fn main() -> CarbonResult<()> {
    ...
    let end_slot = args.end_slot.or_else(|| env::var("BLOCK_CRAWLER_END_SLOT").ok().and_then(|value| value.parse().ok()));
    ...
    let processor = default_clickhouse_processor(config);
