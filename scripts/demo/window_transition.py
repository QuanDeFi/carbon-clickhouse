...
def run_transition(label: str) -> None:
    ...
    # Off-screen by default so Chrome's white window-creation paint happens
    # off-screen (never recorded); the window is moved on-screen only after the
    # dark page below is prepared.
    ...
    proc = subprocess.Popen(
        [
            ...
            "--disable-infobars",
            "--test-type",
            # Keep painting while off-screen so the dark first paint is ready
            # before the overlay is moved on-screen (no white window flash).
