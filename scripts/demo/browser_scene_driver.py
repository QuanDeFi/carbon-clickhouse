...
def main() -> int:
    ...
    if args.mode == "slide":
        ...
        if not path.is_absolute():
            path = ROOT / path
        # Slides are authored dark and the orchestrator starts recording only
        # after this process writes DEMO_SLIDE_READY_FILE. Launch slides
        # on-screen: on this Xvfb rig Chromium app windows expose a separate
        # wrapper when launched off-screen, which can leave the titled content
        # window parked at -32000,-32000 while a black wrapper is moved into
        # frame.
        return run_scene(
            args.scene_id,
            path.resolve().as_uri(),
