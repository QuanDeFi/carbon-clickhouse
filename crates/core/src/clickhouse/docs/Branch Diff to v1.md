...
- Commit relation: `0` commits behind `upstream-v1-sync`, `16` commits ahead
- Changed files: `56`
- Added files: `30`
- Modified files: `26`
- Deleted files: `0`
...
| M | `+45 / -0` | `Cargo.lock` | Locks the ClickHouse core dependencies, decoder feature dependencies, and the two ClickHouse example crates. |
...
| A | `+163 / -0` | `crates/core/src/clickhouse/metrics.rs` | Registers and records ClickHouse instruction/account metrics for rows, failures, buffers, batches, and flush duration. |
| A | `+13 / -0` | `crates/core/src/clickhouse/mod.rs` | Re-exports the ClickHouse runtime modules behind the feature gate. |
| A | `+400 / -0` | `crates/core/src/clickhouse/processors.rs` | Implements generic instruction and account ClickHouse processors, processor finalization, metrics registration, and tests. |
| A | `+109 / -0` | `crates/core/src/clickhouse/rows/mod.rs` | Defines row/table traits, multi-row emission contract, row context, and deterministic instruction/event/account ID helpers. |
...
| A | `+606 / -0` | `decoders/jupiter-swap-decoder/src/instructions/clickhouse/cpi_event_row.rs` | Defines typed Jupiter CPI-event landing row, DDL, structured payload helpers, row conversion, and tests. |
...
| A | `+97 / -0` | `examples/jupiter-swap-clickhouse/src/main.rs` | Runs bounded RPC block crawling into generated Jupiter ClickHouse instruction/event rows. |
...
| M | `+74 / -1` | `packages/renderer/src/getRenderMapVisitor.ts` | Hooks `withClickHouse` into account, instruction, CPI-event row rendering and module generation. |
...
| A | `+195 / -0` | `packages/renderer/templates/eventInstructionClickHouseRowPage.njk` | Generates typed per-event CPI ClickHouse row structs, DDL, conversions, and row trait impls. |
| M | `+17 / -1` | `packages/renderer/templates/eventInstructionPage.njk` | Generates CPI-event account extraction helper from raw instruction accounts. |
| A | `+206 / -0` | `packages/renderer/templates/instructionsClickHouseMod.njk` | Generates instruction/event ClickHouse module dispatch, migrations, setup helpers, and processor wiring. |
| M | `+20 / -4` | `packages/renderer/templates/instructionsMod.njk` | Emits instruction ClickHouse module and moves Anchor CPI-event decode into an explicit pre-check path. |
| A | `+236 / -0` | `packages/renderer/test/clickhouse-renderer.test.cjs` | Tests ClickHouse renderer modules, dependencies, typed row output, structured mapping, and generated DDL. |

## Raw Numstat

The file summary above is derived from `git diff --numstat upstream-v1-sync`. The numeric values are the insertion/deletion counts reported by Git for each changed path.
