use carbon_core::{
    clickhouse::{
        rows::{deterministic_event_id, ClickHouseRow, ClickHouseTable},
        ClickHouseConfig,
    },
...
use crate::{
    events::swap_event::SwapEventEvent,
...
impl JupiterSwapSwapEventLandingRow {
    ...
    pub fn try_from_instruction(
        ...
        decoded_instruction: &DecodedInstruction<JupiterSwapInstruction>,
        config: &ClickHouseConfig,
    ) -> Option<Self> {
        let JupiterSwapInstruction::CpiEvent(cpi_event) = &decoded_instruction.data else {
            return None;
        };
        let CpiEvent::SwapEvent(swap_event) = cpi_event.as_ref() else {
            return None;
        };
        Some(Self::from_parts(metadata, swap_event, config))
    }
    ...
    pub fn from_parts(
        ...
        Self::from_parts_with_ingest_ts(metadata, swap_event, config, Utc::now())
    }
...
mod tests {
    ...
    fn swap_event_row_mapping_is_stable() {
        ...
        assert_eq!(first.family_name, JupiterSwapSwapEventLandingRow::FAMILY_NAME);
